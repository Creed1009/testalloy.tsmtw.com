...
class Ion_auth_model extends CI_Model
{
	...
	/**
	 * @property Identity_column $identity_column
	...
	public function __construct()
	{
		...
		// $group_name = $this->session->userdata('database');
    ...
    function getUsersCount($params = array()){
        ...
        if(!empty($params['search']['keywords'])){
            ...
            $this->db->or_like('member_number',$params['search']['keywords']);
            $this->db->group_end();
        }
        if(!empty($params['search']['category'])){
            $this->db->where('school',$params['search']['category']);
        }
        if(!empty($params['search']['category2'])){
            $this->db->where('certification',$params['search']['category2']);
        }
        // sort data by ascending or desceding order
        // if(!empty($params['search']['sortBy'])){
        //     $this->db->order_by('users.id',$params['search']['sortBy']);
    ...
    function getSubscriptEmailUsers($params = array()){
		$school = $this->ion_auth->user()->row()->school;
        $this->db->select('*');
        $this->db->from('users');
        $this->db->join('users_groups', 'users_groups.user_id = users.id');
